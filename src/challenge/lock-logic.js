...
function changeDialValue (index, incrementBy) {
  ...
  // i'm assuming that the combo lock wheels are supposed to go 0-9 and wrap
  ...
  for (let i = 0; i < SECRET_COMBO.length; i++) {
    if (SECRET_COMBO[i] !== lockState.wheels[i]) {
      return // i assume we can count on this to skip the redirect if the combo is wrong
    }
  }
  lockState.locked = false
