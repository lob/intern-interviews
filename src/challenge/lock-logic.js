...
function changeDialValue(index, incrementBy) {
    ...
    // ----DARWISH QUIAMBAO-----
    lockState.wheels[index] += incrementBy // Increment/Decrement lock by the argument amount (i.e. incrementBy value)

    // Evaluate each element of wheels to match SECRET COMBO.
    for (let i = 0; i < lockState.wheels.length; i++) {
        // if both element from arrays match at same index, set lockedState to false
        if (lockState.wheels[i] === SECRET_COMBO[i]) {
            lockState.locked = false
        }
        // else, reset to locked (i.e. if a mismatched integer is encountered along the way while checking)
        else {
            lockState.locked = true
        }
    }

    // Then, if all combo matches, and lock is unlocked, redirect to page.
    lockState.locked === false ? redirect('darwish-quiambao') : console.log('Status: ' + lockState.locked)

    // --x--DARWISH QUIAMBAO--x--
