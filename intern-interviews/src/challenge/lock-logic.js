...
function changeDialValue(index, incrementBy) {
  ...
  if (lockState.wheels[index] === 0 && incrementBy < 1) return;

  if (lockState.wheels[index] >= 0 && lockState.wheels[index] < 9) {
  ...
  const userValues = [...lockState.wheels];
  const correctValues = userValues.every(val => SECRET_COMBO.includes(val));
  if (correctValues) {
    lockState.locked = false;
    redirect('kevin-espinal');
  }
}
